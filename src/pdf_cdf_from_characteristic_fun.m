function [pdfsCMOS,cdfsCMOS,L,U] = pdf_cdf_from_characteristic_fun(intensities,lambda,gain,shapep,adFactor,offset,roNoise,L,U)
...
if nargin < 8
    ...
    TLvar=(2.0/shapep^2)*((1/(1+2*shapep))-((gamma(1+shapep))^2/gamma(2*shapep+2)));
...
%     U = min(max(intensities),U); % limit to U for truncated case
    ...
    cf = char_fun(t , gain,shapep,roNoise,lambda,offset);
...
function cfCombined = char_fun(t,gain,lambdap,roNoise,lambda,offset)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%lambdap=shapep;
del=0.005;
rn = 0:del:1;
