...
function [outres] = scmos_segmentation(filename, chipParsCur,outFig,pValThresh,pValThreshBinarization)
...
chipPars.shapeParameter = 0.0517;
chipPars.adFactor = 1;
...
for i=1:1
    ...
    im = imread(filename); % Specify target image
    ...
    for idx = 1:size(matrixBlocks,3)
       ...
       image2.imAverage = matrixBlocks(:,:,idx);
        ...
        tic
        [lambdaBg, intThreshBg, stats] = scmospmf_estimation(chipPars,image2,pValThresh,0);
    ...
    shapep=chipPars.shapeParameter;
  ...
  for j=1:size(binarizedImageT,3)
        ...
        [binarizedImageT(:,:,j) , intThreshBlackWhite(j) ] = binarize_image_pval_thresh2(...
            matrixBlocks(:,:,j), pValThreshBinarization , lambdaPars(j) , gain, shapep,adFactor, offset, roNoise);
    end
...
%         M = regSizesImg(regIdx);
       ...
       [~,cdfTemp,~,~] = pdf_cdf_from_characteristic_fun_sum( summedInt(regIdx), mean(lambdavals(:)),gain,shapep,adFactor, offset,roNoise,M); 
